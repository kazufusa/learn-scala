              ...
              case Right(u) =>
                UserRepo
                  ...
                  .foldCauseZIO(
                    failure =>
                      ZIO
                        .logErrorCause(s"Failed to register user", Cause.fail(failure))
                        .as(Response.status(Status.InternalServerError)),
                    success => ZIO.logInfo(s"User registered: $success").as(Response.text(success))
                  )
            }
          } yield r
                ...
                .foldZIO(
                  {
                    case Some(error) =>
                      ZIO
                        .logErrorCause(s"Failed to lookup user", Cause.fail(error))
                        .as(Response.status(Status.InternalServerError))
      ...
      case req @ (Method.GET -> !! / "users") => {
          for {
            ...
            users <- UserRepo.users.foldCauseZIO(
              failure =>
                ZIO
                  .logErrorCause(s"Failed to retrieve users.", failure)
                  .as(Response.status(Status.InternalServerError)),
              success =>
                ZIO
                  .log(s"Retrieved users successfully: response length=${success.length}")
